...
from utils2 import create_logger, create_directory, increment_path, save_performance_graph, Evaluator, get_optimizer
from dataset2 import preprocess
...
#         # weight
#     data = data.digit_3.value_counts().to_frame().reset_index().sort_values(['index']).reset_index(drop=True).reset_index()
...
#         weights_for_samples2 = weights_for_samples/np.sum(weights_for_samples)*no_of_classes
#         return weights_for_samples2
    
#     y_cnt = data["digit_3"].tolist()
#     weights = get_weights_inverse_num_of_samples(225, y_cnt, power=1)
    
#     # Loss 가중치 정의
#     criterion = criterion(weight = torch.FloatTensor(weights).to(args.device))
